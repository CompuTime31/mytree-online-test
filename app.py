...
APP_VERSION='v2.0 Alpha 1 — Multi-Associations Online Test'
...
def migrate_legacy(c):
 ...
 for table in ['projects','zones','teams','missions','events','trees','members','donations','cash_movements','agents','agent_payments','purchase_records','purchase_groups','operational_tasks']:
...
def api_project_zones(pid):
 c=db(); rows=c.execute('SELECT id,name FROM zones WHERE project_id=? AND active=1 ORDER BY name',(pid,)).fetchall(); c.close(); return jsonify([dict(x) for x in rows])
...
def team_new():
 ...
 c=db(); opts=filter_options(c); leaders=c.execute('SELECT id,name FROM users WHERE active=1 ORDER BY name').fetchall(); volunteers=c.execute("SELECT id,name,phone FROM users WHERE active=1 ORDER BY name").fetchall(); suggested=next_entity_code(c,'teams','code','EQUIPE')
 if request.method=='POST':
  now=datetime.now().isoformat(timespec='minutes'); code=clean(request.form.get('code')) or suggested; cur=c.execute('INSERT INTO teams(code,name,leader_user_id,project_id,zone_id,phone,mission,active,created_by_user_id,created_at,updated_at) VALUES(?,?,?,?,?,?,?,1,?,?,?)',(code,request.form['name'].strip(),request.form.get('leader_user_id') or None,request.form.get('project_id') or None,request.form.get('zone_id') or None,request.form.get('phone'),request.form.get('mission'),session['uid'],now,now)); tid=cur.lastrowid
...
def mission_new():
 ...
 c=db(); opts=filter_options(c); teams=c.execute('SELECT * FROM teams WHERE active=1 ORDER BY name').fetchall(); leaders=c.execute('SELECT id,name FROM users WHERE active=1 ORDER BY name').fetchall(); volunteers=c.execute("SELECT u.id,u.name FROM users u LEFT JOIN roles r ON r.id=u.role_id WHERE u.active=1 AND COALESCE(r.name,u.role) IN ('volunteer','team_leader','coordinator') ORDER BY u.name").fetchall(); suggested=next_entity_code(c,'missions','code','MISSION')
 if request.method=='POST':
  now=datetime.now().isoformat(timespec='minutes'); code=clean(request.form.get('code')) or suggested
  cur=c.execute("INSERT INTO missions(code,title,mission_type,status,priority,project_id,zone_id,team_id,leader_user_id,start_at,end_at,target_count,completed_count,description,report,latitude,longitude,active,created_by_user_id,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,0,?,NULL,?,?,1,?,?,?)",(code,request.form['title'].strip(),request.form.get('mission_type'),request.form.get('status') or 'Planifiée',request.form.get('priority') or 'Normale',request.form.get('project_id') or None,request.form.get('zone_id') or None,request.form.get('team_id') or None,request.form.get('leader_user_id') or None,request.form.get('start_at'),request.form.get('end_at'),request.form.get('target_count') or 0,request.form.get('description'),request.form.get('latitude') or None,request.form.get('longitude') or None,session['uid'],now,now)); mid=cur.lastrowid
...
def admin_association_new():
 ...
 if request.method=='POST':
  ...
  c.execute("INSERT INTO associations(code,name,short_name,description,wilaya_id,commune_id,address,phone,email,map_symbol,status,created_by_user_id,created_at) VALUES(?,?,?,?,?,?,?,?,?,?, 'active',?,?)",(association_code(c),name,clean(request.form.get('short_name')),clean(request.form.get('description')),request.form.get('wilaya_id') or None,request.form.get('commune_id') or None,clean(request.form.get('address')),clean(request.form.get('phone')),clean(request.form.get('email')),clean(request.form.get('map_symbol')) or '🌳',session['uid'],now)); c.commit(); c.close(); flash('Association créée.'); return redirect('/admin/associations')
 c=db(); wilayas=c.execute('SELECT * FROM wilayas WHERE active=1 ORDER BY name').fetchall(); communes=c.execute('SELECT * FROM communes WHERE active=1 ORDER BY name').fetchall(); c.close()
