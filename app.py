...
def volunteer_trees():
 ...
 <div class="mobile-only mobile-tree-list-v1615">
  ...
  <div class="card mobile-tree-card"><b>{{mobile_rows|length}} arbre(s) trouvé(s)</b><div class="mobile-tree-table-wrap"><table class="mobile-tree-table"><tr><th>Code</th><th>Espèce</th><th>Origine</th><th>Projet / Zone</th><th></th></tr>{% for t in mobile_rows %}<tr onclick="location.href='/tree/{{t.id}}'" tabindex="0" role="link" onkeydown="if(event.key==='Enter')location.href='/tree/{{t.id}}'"><td><b>{{t.tree_code or '—'}}</b></td><td>{{t.species_name or '—'}}</td><td>{{t.association_name or 'Individuel'}}</td><td>{{t.project_name or '—'}} / {{t.zone_name or '—'}}</td><td>›</td></tr>{% else %}<tr><td colspan="5">Aucun arbre.</td></tr>{% endfor %}</table></div></div>
 </div>
 <style>
 @media(max-width:760px){.mobile-tree-list-v1615{display:block!important}.mobile-tree-tabs{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:8px 0 14px}.mobile-tree-tab{display:flex;min-height:58px;align-items:center;justify-content:center;text-decoration:none;border:1px solid var(--line);border-radius:14px;background:#fff;color:var(--deep);font-weight:800;text-align:center;padding:8px}.mobile-tree-tab.active{background:var(--green);color:#fff;border-color:var(--green)}.mobile-tree-card{padding:14px 10px}.mobile-tree-table-wrap{overflow-x:auto;margin-top:12px}.mobile-tree-table{min-width:620px!important}.mobile-tree-table tr:not(:first-child){cursor:pointer}.mobile-tree-table td:last-child{font-size:25px;font-weight:700}.mobile-tree-table td:first-child{color:var(--green)}}
 </style>
