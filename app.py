...
# RC16.18.3.7 — Tree Grove Clusters
...
def ensure_demo_database(force=False):
 # RC16.18.3.6: the Demo environment must remain self-healing on Railway/Windows.
...
APP_VERSION='v2.0 Alpha 4 — RC16.18.3.7 — Tree Grove Clusters'
...
def _system_request_lang():
 # Browser/phone language. Unsupported languages intentionally fall back to English.
 raw=(request.headers.get('Accept-Language') or '').lower()
 primary=(raw.split(',')[0].split(';')[0].strip().split('-')[0] if raw else '')
 return primary if primary in SUPPORTED_LANGS else 'en'

def current_lang():
 mode=request.cookies.get('mytree_lang_mode','system')
 manual=request.cookies.get('mytree_lang')
 if mode=='manual' and manual in SUPPORTED_LANGS:
  return manual
 lang=session.get('lang')
 if mode=='manual' and lang in SUPPORTED_LANGS:
  return lang
 return _system_request_lang()
...
def language_switcher():
 ...
 mode=request.cookies.get('mytree_lang_mode','system')
 for code,label in [('system','🌐 Auto'),('fr','FR'),('ar','العربية'),('en','EN')]:
  cls='lang-active' if (code=='system' and mode!='manual') or (code==lang and mode=='manual') else ''
  out.append(f'<a class="lang-link {cls}" href="/language/{code}?next={back}">{label}</a>')
 return '<div class="lang-switch mobile-language-switch">'+''.join(out)+'</div>'
...
def set_language(lang):
 ...
 if lang=='system':
  ...
  resp.set_cookie('mytree_lang_mode','system',max_age=365*24*3600,samesite='Lax')
  resp.delete_cookie('mytree_lang')
  return resp
 ...
 resp=redirect(target)
 resp.set_cookie('mytree_lang_mode','manual',max_age=365*24*3600,samesite='Lax')
 resp.set_cookie('mytree_lang',lang,max_age=365*24*3600,samesite='Lax')
 return resp
...
/* RC16.17.6 — Mobile Map Fullscreen UX + visible Ma carte */
@media(max-width:700px){
  .mobile-map-page{margin:-12px -10px -12px;position:relative;height:calc(100dvh - 138px);min-height:560px;overflow:hidden;background:#eef5ef}
  .mobile-map-page>.section-title{display:none!important}
  .mobile-map-page .map-scope-bar,.mobile-map-page .map-action-bar,.mobile-map-page .map-location-bar{position:absolute;z-index:700;top:10px;margin:0;display:flex;gap:5px;flex-wrap:nowrap;width:auto;background:transparent;padding:0}
  .mobile-map-page .map-scope-bar{left:8px}
  .mobile-map-page .map-action-bar{left:205px}
  .mobile-map-page .map-location-bar{right:8px}
  .mobile-map-page .map-filter-bar .btn{width:auto!important;min-width:0!important;padding:8px 8px!important;border-radius:999px!important;font-size:10.5px!important;line-height:1.1;white-space:nowrap;box-shadow:0 3px 12px rgba(16,43,28,.16);border:1px solid rgba(255,255,255,.75)}
  .mobile-map-page .map-scope-bar .btn{display:inline-flex!important;align-items:center;justify-content:center}
  .mobile-map-page .map-scope-bar .btn:first-child{max-width:104px}
  .mobile-map-page .map-scope-bar .btn:nth-child(2){max-width:86px}
  .mobile-map-page .map-action-bar .btn{max-width:76px}
  .mobile-map-page .map-location-bar .btn{max-width:98px}
  .mobile-map-page .map-action-bar .badge{position:absolute;top:46px;left:-122px;white-space:nowrap;padding:7px 10px;font-size:11px;box-shadow:0 3px 10px rgba(16,43,28,.12)}
  .mobile-map-page .map-action-bar a.btn.alt{position:absolute;top:45px;left:-15px;width:32px!important;height:32px;padding:7px!important;font-size:0!important;border-radius:50%!important}
  .mobile-map-page .map-action-bar a.btn.alt::after{content:'✕';font-size:14px}
  .mobile-map-page .map-result-count{position:absolute;z-index:690;top:56px;right:10px;margin:0;padding:6px 9px;border-radius:999px;background:rgba(255,255,255,.9);font-size:11px;box-shadow:0 2px 10px rgba(16,43,28,.12)}
  .mobile-map-page .map-layout{display:block!important;height:100%;margin:0}
  .mobile-map-page .map-card{position:absolute;inset:0;margin:0!important;padding:0!important;border:0!important;border-radius:0!important;overflow:hidden!important}
  .mobile-map-page .real-map{height:100%!important;min-height:100%!important;border:0!important;border-radius:0!important}
  .mobile-map-page .map-nearby-card{display:none!important}
  .mobile-map-page .map-filter-drawer{position:absolute;z-index:1200;left:10px;right:10px;top:64px;max-height:calc(100% - 82px);overflow:auto;margin:0;border-radius:16px;box-shadow:0 10px 34px rgba(16,43,28,.25)}
  .mobile-map-page .leaflet-control-zoom{margin-top:94px!important}
  .mobile-map-page .leaflet-bottom{bottom:4px}
}
</style>'''
...
<div class="admin-nav-block"><div class="admin-nav-title">💬 Communication</div><a href="/messages">💬 Messagerie</a><a href="/suggestions">💡 Suggestions</a></div>
...
STYLE += '''<style id="rc16172-web-messaging-suggestions-nav">
@media(max-width:700px){
 .layout>aside.vol-nav{display:none!important}
 .layout{padding-bottom:132px!important}
 .mobile-connected-nav{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;height:auto!important;min-height:116px!important;padding:5px 4px max(5px,env(safe-area-inset-bottom))!important;gap:3px!important}
 .mobile-connected-nav a{display:flex!important;flex-direction:column;align-items:center;justify-content:center;min-height:50px!important;font-size:9.5px!important;padding:4px 1px!important}
 .mobile-connected-nav a span{font-size:19px!important;line-height:20px!important}
 .rc16172-communication{border:2px solid #2e7b47!important;background:#f4fbf6!important}
 .rc16172-new{border:2px solid #2e7b47!important;background:#f4fbf6!important}
}
</style>'''
...
<div class="admin-home-block rc16172-communication"><h3>💬 Communication</h3><div class="admin-home-links"><a href="/messages">💬 Messagerie</a><a href="/suggestions">💡 Suggestions</a></div></div>
...
def api_map_data():
 ...
 if 'tree' in types:
  ...
  # RC16.18.3.6: never materialize the full tree set in Python for the map.
  # Apply the association scope directly in SQL, aggregate in SQLite, and hard-cap
  # the number of graphical objects returned to Safari/Android WebView.
  if ctx.get('type')=='association' and request.args.get('scope')!='global':
   aid=ctx.get('association_id')
   accessible_ids=sorted(project_ids)
   if accessible_ids:
    marks=','.join('?'*len(accessible_ids))
    where.append('(t.association_id=? OR t.project_id IN ('+marks+'))')
    args += [aid] + accessible_ids
   else:
    where.append('t.association_id=?'); args.append(aid)
   base_where=' AND '.join(where)
  count_row=c.execute("SELECT COUNT(*) n FROM trees t WHERE "+base_where,args).fetchone()
  total_trees=int(count_row['n'] or 0)
  if zoom < 15:
   factor=40 if zoom<=11 else (80 if zoom<=12 else (160 if zoom<=13 else 320))
   cluster_limit=160
   q=("SELECT ROUND(t.latitude*?,0) gx,ROUND(t.longitude*?,0) gy,"
      "COUNT(*) n,AVG(t.latitude) latitude,AVG(t.longitude) longitude "
      "FROM trees t WHERE "+base_where+" GROUP BY gx,gy ORDER BY n DESC LIMIT ?")
   grouped=c.execute(q,[factor,factor]+args+[cluster_limit]).fetchall()
   for i,r in enumerate(grouped,1):
    n=int(r['n'] or 0)
    data.append({'type':'cluster','id':'cluster-'+str(i),'lat':r['latitude'],'lon':r['longitude'],
                 'title':str(n)+' arbres','subtitle':'Touchez pour zoomer','url':'','count':n})
   tree_groups=len(grouped)
   truncated=tree_groups>=cluster_limit
  else:
   limit=300
   ...
   for r in c.execute(q,args+[limit]).fetchall():
    add('tree',r,r['tree_code'] or 'Arbre',(r['species_name'] or r['species'] or '')+' · '+(r['project_name'] or 'Hors projet'),'/tree/'+str(r['id']))
...
def real_map():
 ...
 c.close()
 return page('Carte commune',"""<div class='mobile-map-page'><div class='section-title'><div><h2>🗺 Carte commune</h2><p class='sub'>Par défaut, seuls les arbres sont affichés. Ajoutez Zones ou Événements depuis Filtrer.</p></div></div><div class='map-filter-bar map-scope-bar noprint'>{% if ctx.type=='personal' %}<a class='btn {% if f.quick!="mine" %}active{% else %}alt{% endif %}' href='/map'>🌐 Carte globale</a><a class='btn {% if f.quick=="mine" %}active{% else %}alt{% endif %}' href='/map?quick=mine'>🗺 Ma carte</a>{% elif ctx.type=='association' %}<a class='btn {% if request.args.get("scope")!="global" %}active{% else %}alt{% endif %}' href='/map'>🏛 Carte Association</a><a class='btn {% if request.args.get("scope")=="global" %}active{% else %}alt{% endif %}' href='/map?scope=global'>🌐 Carte globale</a>{% endif %}</div><div class='map-filter-bar map-action-bar noprint'><button class='btn' type='button' id='toggleMapFilters'>🔎 Filtrer</button>{% if f.quick=='mine' %}<span class='badge good'>👤 Mes arbres ✓</span><a class='btn alt' href='/map'>✕ Supprimer le filtre</a>{% endif %}<span id='activeFilterSummary' class='sub'></span></div><form id='mapFilters' class='card map-filter-drawer noprint' method='get' aria-hidden='true'><div class='section-title'><h3>🔎 Filtres de la carte</h3><button class='btn alt' type='button' id='closeMapFilters'>Fermer</button></div>{% if f.quick %}<input type='hidden' name='quick' value='{{f.quick}}'>{% endif %}<div class='card'><b>Afficher sur la carte</b><div class='map-layer-choices'><label><input type='checkbox' name='type' value='tree' checked> 🌳 Arbres</label><label><input type='checkbox' name='type' value='zone' {% if 'zone' in request.args.getlist('type') %}checked{% endif %}> 🟩 Zones</label><label><input type='checkbox' name='type' value='event' {% if 'event' in request.args.getlist('type') %}checked{% endif %}> 📆 Événements</label></div></div><div class='map-filter-grid'><label>Association<select name='association_id'><option value=''>Toutes</option>{% for a in associations %}<option value='{{a.id}}' {% if f.association_id|string==a.id|string %}selected{% endif %}>{{a.map_symbol or '🌳'}} {{a.name}}</option>{% endfor %}</select></label><label>Projet<select name='project_id'><option value=''>Tous</option>{% for p in projects %}<option value='{{p.id}}' {% if f.project_id|string==p.id|string %}selected{% endif %}>{{p.name}}</option>{% endfor %}</select></label><label>Zone<select name='zone_id'><option value=''>Toutes</option>{% for z in zones %}<option value='{{z.id}}' {% if f.zone_id|string==z.id|string %}selected{% endif %}>{{z.name}}</option>{% endfor %}</select></label><label>Espèce<select name='species_id'><option value=''>Toutes</option>{% for x in species %}<option value='{{x.id}}' {% if f.species_id|string==x.id|string %}selected{% endif %}>{{x.name_fr}}</option>{% endfor %}</select></label><label>Bénévole<select name='volunteer_id'><option value=''>Tous</option>{% for x in volunteers %}<option value='{{x.id}}' {% if f.volunteer_id|string==x.id|string %}selected{% endif %}>{{x.name}}</option>{% endfor %}</select></label><label>Santé<select name='health_status'><option value=''>Toutes</option>{% for x in ['Bon','À surveiller','En danger','Mort'] %}<option {% if f.health_status==x %}selected{% endif %}>{{x}}</option>{% endfor %}</select></label><label>Arrosage<select name='watering_status'><option value=''>Tous</option>{% for x in ['À jour','À arroser','Urgent'] %}<option {% if f.watering_status==x %}selected{% endif %}>{{x}}</option>{% endfor %}</select></label></div><div class='map-filter-actions'><button class='btn'>Appliquer</button><a class='btn alt' href='/map'>Réinitialiser</a></div></form><div class='map-filter-bar map-location-bar noprint'><button class='btn alt' type='button' id='locateBtn'>📍 Ma position</button></div><div class='card map-result-count'><b id='resultCount'>Chargement…</b></div><style id='rc161837-groves'>.map-grove-icon,.map-single-tree-icon{background:transparent!important;border:0!important}.grove-wrap{position:relative;display:block;width:100%;height:100%;filter:drop-shadow(0 2px 2px rgba(0,0,0,.22))}.grove-tree{position:absolute;font-size:24px;line-height:1;transform-origin:center bottom}.grove-tree-0{left:50%;top:3px;transform:translateX(-50%) scale(1.05)}.grove-tree-1{left:6px;top:18px;transform:rotate(-6deg) scale(.9)}.grove-tree-2{right:6px;top:18px;transform:rotate(6deg) scale(.9)}.grove-tree-3{left:13px;top:1px;transform:scale(.74)}.grove-tree-4{right:13px;top:1px;transform:scale(.74)}.grove-tree-5{left:1px;top:7px;transform:scale(.66)}.grove-tree-6{right:1px;top:7px;transform:scale(.66)}.grove-count{position:absolute;right:-3px;bottom:-3px;min-width:22px;height:22px;padding:0 5px;border-radius:11px;background:#fff;color:#173c28;border:2px solid #2f874f;font:700 12px/18px system-ui,-apple-system,sans-serif;text-align:center;box-shadow:0 1px 3px rgba(0,0,0,.2)}.grove-size-3 .grove-tree:nth-child(n+4),.grove-size-5 .grove-tree:nth-child(n+6){display:none}.single-tree{display:block;font-size:28px;line-height:30px;filter:drop-shadow(0 1px 2px rgba(0,0,0,.22))}@media(max-width:720px){.grove-tree{font-size:22px}.grove-count{font-size:11px;min-width:21px;height:21px;line-height:17px}}</style><div class='grid two map-layout'><div class='card map-card'><div id='map' class='real-map'></div></div><div class='card map-nearby-card'><h3>Éléments proches</h3><div id='locationStatus' class='sub'>Utilisez « Ma position » pour calculer les distances.</div><div id='nearbyList'></div></div></div><script>(function(){const qp=new URLSearchParams(location.search);if(!qp.getAll('type').length)qp.append('type','tree');const box=mapFilters,open=toggleMapFilters,close=closeMapFilters;function drawer(v){box.classList.toggle('open',v);box.setAttribute('aria-hidden',v?'false':'true');(v?close:open).focus()}open.onclick=()=>drawer(true);close.onclick=()=>drawer(false);document.addEventListener('keydown',e=>{if(e.key==='Escape'&&box.classList.contains('open'))drawer(false)});const map=L.map('map',{preferCanvas:true,zoomAnimation:false,fadeAnimation:false,markerZoomAnimation:false}).setView([35.697,-0.633],11),group=L.featureGroup().addTo(map),canvasRenderer=L.canvas({padding:.2,tolerance:4});L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:20,attribution:'&copy; OpenStreetMap',updateWhenIdle:true,keepBuffer:0}).addTo(map);let items=[],me=null,meMarker=null,loadTimer=null,loadSeq=0,loadController=null;const icons={tree:'🌳',zone:'🟩',event:'📆',cluster:'🌳'};const esc=v=>String(v??'—').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));function pop(x){let h='<b>'+icons[x.type]+' '+esc(x.title)+'</b><br>'+esc(x.subtitle);if(x.url)h+='<br><a href="'+x.url+'">Voir la fiche</a>';if(x.type==='tree')h+='<br><a target="_blank" rel="noopener" href="https://www.google.com/maps/dir/?api=1&destination='+encodeURIComponent(x.lat+','+x.lon)+'">📍 Itinéraire</a>';return h}function groveHtml(n){let trees=n<10?Math.min(3,n):n<50?3:n<200?5:7;let glyph='';for(let i=0;i<trees;i++)glyph+='<span class="grove-tree grove-tree-'+i+'">🌳</span>';return '<span class="grove-wrap grove-size-'+trees+'">'+glyph+'<span class="grove-count">'+n+'</span></span>'}function draw(x){if(x.type==='cluster'){const n=x.count||1,size=n<10?48:n<50?52:n<200?58:64;const ic=L.divIcon({className:'map-grove-icon',html:groveHtml(n),iconSize:[size,size],iconAnchor:[Math.round(size/2),Math.round(size/2)]});const mk=L.marker([x.lat,x.lon],{icon:ic,keyboard:false,riseOnHover:false}).addTo(group);mk.on('click',()=>map.setView([x.lat,x.lon],Math.min(17,map.getZoom()+2)));return}if(x.type==='tree'){const ic=L.divIcon({className:'map-single-tree-icon',html:'<span class="single-tree" aria-label="Arbre">🌳</span>',iconSize:[30,34],iconAnchor:[15,29]});L.marker([x.lat,x.lon],{icon:ic,keyboard:false,riseOnHover:false}).bindPopup(pop(x)).addTo(group);return}const ic=L.divIcon({className:'tree-emoji-marker map-common-marker',html:'<span>'+icons[x.type]+'</span>',iconSize:[32,32],iconAnchor:[16,27]});L.marker([x.lat,x.lon],{icon:ic}).bindPopup(pop(x)).addTo(group)}function loadMap(){clearTimeout(loadTimer);loadTimer=setTimeout(()=>{const seq=++loadSeq,b=map.getBounds(),q=new URLSearchParams(qp);q.set('zoom',String(map.getZoom()));q.set('south',b.getSouth().toFixed(6));q.set('west',b.getWest().toFixed(6));q.set('north',b.getNorth().toFixed(6));q.set('east',b.getEast().toFixed(6));if(loadController)loadController.abort();loadController=new AbortController();resultCount.textContent='Chargement de la zone visible…';fetch('/api/map-data?'+q,{cache:'no-store',signal:loadController.signal}).then(r=>r.ok?r.json():Promise.reject(new Error('http'))).then(p=>{if(seq!==loadSeq)return;group.clearLayers();items=(p.items||[]).slice(0,320);for(const x of items)draw(x);if(p.tree_groups>0)resultCount.textContent=(p.tree_total||0)+' arbre(s) regroupé(s) en '+p.tree_groups+' groupe(s) — zoomez pour afficher les arbres.';else if(p.truncated)resultCount.textContent=(p.tree_total||0)+' arbre(s) dans cette zone — affichage limité pour protéger les performances. Zoomez davantage.';else resultCount.textContent=items.length+' élément(s) dans la zone visible.'}).catch(e=>{if(e&&e.name==='AbortError')return;if(seq===loadSeq)resultCount.textContent='Impossible de charger la carte. Réessayez ou zoomez sur une zone plus petite.'})},300)}map.on('moveend',loadMap);setTimeout(()=>{map.invalidateSize(false);loadMap()},80);locateBtn.onclick=()=>navigator.geolocation&&navigator.geolocation.getCurrentPosition(p=>{me={lat:p.coords.latitude,lng:p.coords.longitude};if(meMarker)map.removeLayer(meMarker);meMarker=L.marker(me).addTo(map).bindPopup('Votre position');map.setView(me,16);locationStatus.textContent='Position obtenue.'},()=>locationStatus.textContent='Position refusée ou indisponible.');})();</script></div>""",ctx=ctx,f=f,**opts)
...
@login_required
def volunteer_trees():
 # RC16.15 — Web téléphone uniquement : écran Arbres simplifié.
 # Le rendu desktop historique reste intact dans .desktop-only.
 ...
 mobile_view='mine' if request.args.get('view')=='mine' else 'all'
 mobile_sql="""SELECT t.*,s.name_fr species_name,z.name zone_name,p.name project_name,u.name volunteer_name,a.name association_name
  FROM trees t LEFT JOIN species s ON s.id=t.species_id LEFT JOIN zones z ON z.id=t.zone_id LEFT JOIN projects p ON p.id=t.project_id LEFT JOIN users u ON u.id=t.planted_by_user_id LEFT JOIN associations a ON a.id=t.association_id
  WHERE t.active=1 AND t.approval_status='approved'"""
 mobile_params=[]
 if mobile_view=='mine':
  mobile_sql += " AND t.planted_by_user_id=?"; mobile_params.append(session.get('uid'))
 mobile_sql += " ORDER BY t.id DESC"
 mobile_rows=c.execute(mobile_sql,mobile_params).fetchall(); c.close()
 opts.update(volunteers=volunteers,associations=assocs)
 return page('Arbres',"""
 <div class="desktop-only">
  <div class="section-title"><div><h2>🌳 Mes arbres</h2><p class="sub">Vos arbres et vos accès rapides.</p></div><div class="section-actions"><a class="btn" href="/map?quick=mine">🗺️ Ma carte</a><a class="btn" href="/planting/new">+ Planter</a></div></div>
  <div class="filter-quick card"><a class="btn alt" href="/volunteer/trees">Tous</a><a class="btn alt" href="/volunteer/trees?quick=mine">Mes arbres</a><a class="btn alt" href="/volunteer/trees?quick=individuals">Individuels</a><a class="btn alt" href="/volunteer/trees?quick=associations">Associations</a><a class="btn alt" href="/volunteer/trees?quick=watering">À arroser</a><a class="btn" href="/map?{{request.query_string.decode()}}">🗺 Carte avec ces filtres</a><button class="btn" onclick="advancedFilters.classList.toggle('open')">⚙️ Plus de filtres</button></div>
  <form id="advancedFilters" class="filter-panel card" method="get"><input type="hidden" name="quick" value="{{f.quick}}"><div class="form"><label>Type<select name="owner_type"><option value="">Tous</option><option value="individual">Individuels</option><option value="association">Associations</option></select></label><label>Association<select name="association_id"><option value="">Toutes</option>{% for x in associations %}<option value="{{x.id}}">{{x.name}}</option>{% endfor %}</select></label><label>Bénévole<select name="volunteer_id"><option value="">Tous</option>{% for x in volunteers %}<option value="{{x.id}}">{{x.name}}</option>{% endfor %}</select></label><label>Projet<select name="project_id"><option value="">Tous</option>{% for x in projects %}<option value="{{x.id}}">{{x.name}}</option>{% endfor %}</select></label><label>Zone<select name="zone_id"><option value="">Toutes</option>{% for x in zones %}<option value="{{x.id}}">{{x.name}}</option>{% endfor %}</select></label><label>Espèce<select name="species_id"><option value="">Toutes</option>{% for x in species %}<option value="{{x.id}}">{{x.name_fr}}</option>{% endfor %}</select></label><label>Arrosage<select name="watering_status"><option value="">Tous</option><option>À jour</option><option>À arroser</option><option>Urgent</option></select></label></div><div class="filter-actions"><a class="btn alt" href="/volunteer/trees">Réinitialiser</a><button class="btn">Appliquer</button></div></form>
  <div class="card"><b>{{rows|length}} arbre(s) trouvé(s)</b><div style="overflow:auto"><table><tr><th>Code</th><th>Espèce</th><th>Origine</th><th>Projet / Zone</th><th>État</th><th></th></tr>{% for t in rows %}<tr><td>{{t.tree_code or '—'}}</td><td>{{t.species_name}}</td><td>{{t.association_name or ('Individuel — '+(t.volunteer_name or 'Bénévole'))}}</td><td>{{t.project_name or '—'}} / {{t.zone_name or '—'}}</td><td>{{t.health_status}}<br><span class="sub">{{t.watering_status}}</span></td><td><a class="btn alt" href="/tree/{{t.id}}">Fiche</a></td></tr>{% else %}<tr><td colspan="6">Aucun arbre avec ces filtres.</td></tr>{% endfor %}</table></div></div>
 </div>
 <div class="mobile-only mobile-tree-list-v16151">
  <div class="section-title"><div><h2>🌳 Liste des arbres</h2></div></div>
  <div class="mobile-tree-tabs"><a class="mobile-tree-tab {% if mobile_view=='all' %}active{% endif %}" href="/volunteer/trees">🌳 Tous les arbres</a><a class="mobile-tree-tab {% if mobile_view=='mine' %}active{% endif %}" href="/volunteer/trees?view=mine">👤 Mes arbres</a></div>
  <div class="card mobile-tree-card"><b>{{mobile_rows|length}} arbre(s) trouvé(s)</b><div class="mobile-tree-table-wrap"><table class="mobile-tree-table"><tr><th>Code</th><th>Espèce</th><th>Origine</th><th></th></tr>{% for t in mobile_rows %}<tr onclick="location.href='/tree/{{t.id}}'" tabindex="0" role="link" onkeydown="if(event.key==='Enter'||event.key===' ')location.href='/tree/{{t.id}}'"><td><b>{{t.tree_code or '—'}}</b></td><td>{{t.species_name or '—'}}</td><td>{{t.association_name or 'Individuel'}}</td><td aria-label="Ouvrir">›</td></tr>{% else %}<tr><td colspan="4">Aucun arbre.</td></tr>{% endfor %}</table></div></div>
 </div>
 <style>
 @media(max-width:760px){.desktop-only{display:none!important}.mobile-only.mobile-tree-list-v16151{display:block!important}.mobile-tree-tabs{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:8px 0 14px}.mobile-tree-tab{display:flex;min-height:58px;align-items:center;justify-content:center;text-decoration:none;border:1px solid var(--line);border-radius:14px;background:#fff;color:var(--deep);font-weight:800;text-align:center;padding:8px}.mobile-tree-tab.active{background:var(--green);color:#fff;border-color:var(--green)}.mobile-tree-card{padding:14px 10px}.mobile-tree-table-wrap{overflow:hidden;margin-top:12px}.mobile-tree-table{width:100%!important;min-width:0!important;table-layout:fixed}.mobile-tree-table th,.mobile-tree-table td{padding:13px 7px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.mobile-tree-table th:nth-child(1),.mobile-tree-table td:nth-child(1){width:27%}.mobile-tree-table th:nth-child(2),.mobile-tree-table td:nth-child(2){width:30%}.mobile-tree-table th:nth-child(3),.mobile-tree-table td:nth-child(3){width:36%}.mobile-tree-table th:nth-child(4),.mobile-tree-table td:nth-child(4){width:7%;text-align:center}.mobile-tree-table tr:not(:first-child){cursor:pointer}.mobile-tree-table td:last-child{font-size:25px;font-weight:700}.mobile-tree-table td:first-child{color:var(--green)}}
 </style>
 """,rows=rows,mobile_rows=mobile_rows,mobile_view=mobile_view,f=f,**opts)
...
def operations_map():
 ...
 return page('Carte opérationnelle',"""<div class='section-title'><div><h2>Carte opérationnelle</h2><p class='sub'>Interventions planifiées et arbres prioritaires.</p></div><a class='btn alt' href='/operations'>Calendrier</a></div><div class='card'><div id='opMap' class='real-map' style='height:650px'></div></div><script>const tasks={{tasks_json|safe}},trees={{trees_json|safe}};const map=L.map('opMap').setView([35.697,-0.633],11);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:20,attribution:'© OpenStreetMap'}).addTo(map);const bounds=[];tasks.forEach(x=>{L.marker([x.latitude,x.longitude]).addTo(map).bindPopup(`<b>${x.title}</b><br>${x.project_name||''} — ${x.zone_name||''}<br>${x.start_at}<br>${x.status}`);bounds.push([x.latitude,x.longitude])});trees.forEach(x=>{L.circleMarker([x.latitude,x.longitude],{radius:7,color:'#bd4747',fillOpacity:.85}).addTo(map).bindPopup(`<b>${x.tree_code||'Arbre'}</b><br>${x.species||''}<br>${x.health_status} • ${x.watering_status}<br><a href='/tree/${x.id}'>Ouvrir</a>`);bounds.push([x.latitude,x.longitude])});if(bounds.length)map.fitBounds(bounds,{padding:[25,25],maxZoom:16});</script>""",tasks_json=json.dumps([dict(x) for x in tasks],ensure_ascii=False),trees_json=json.dumps([dict(x) for x in urgent],ensure_ascii=False))
...
def tenant_guard_alpha4():
 ...
 if r and not tenant_access_allowed(ctx,r['association_id']):
  # RC16.15.2 Web téléphone: un bénévole peut consulter en lecture la fiche d'un arbre visible, même rattaché à une association.
  ua=(request.headers.get('User-Agent') or '').lower()
  mobile_web=any(x in ua for x in ('iphone','android','mobile'))
  if table=='trees' and request.method=='GET' and mobile_web:
   c.close(); return None
...
def api_v1_version():
 return jsonify({'ok':True,'version':'RC16.17.2-Web-Messaging-Suggestions-Buttons','association_public_detail':True,'association_join':True,'messaging':True,'suggestions':True})
...
def association_account_dashboard():
 ...
 return page('Association',"""<div class='section-title'><div><h2>🏛 {{a.name}}</h2><p class='sub'>Espace Association · {{a.code}}</p></div></div><div class='grid kpis'><a class='card kpi' href='/membership-requests'><small>Membres</small><b>{{counts.members}}</b><span>{{counts.pending}} demande(s)</span></a><a class='card kpi' href='/projects'><small>Projets</small><b>{{counts.projects}}</b></a><a class='card kpi' href='/trees'><small>Arbres</small><b>{{counts.trees}}</b></a></div><div class='card rc16172-communication'><h3>💬 Communication</h3><div class='action-set'><a class='btn' href='/messages'>💬 Messagerie</a><a class='btn' href='/suggestions'>💡 Suggestions</a></div></div><div class='card'><h3>Gestion de l’association</h3><div class='action-set'><a class='btn' href='/membership-requests'>Adhérents & rôles</a><a class='btn' href='/donations'>Dons</a><a class='btn' href='/projects'>Projets</a><a class='btn' href='/zones'>Zones</a><a class='btn' href='/nursery'>Stock / Pépinière</a><a class='btn' href='/equipment'>Inventaire matériel</a><a class='btn' href='/reports/operations'>Finances & rapports</a><a class='btn' href='/messages'>💬 Messagerie</a><a class='btn' href='/suggestions'>💡 Suggestions</a><a class='btn alt' href='/public/associations/{{a.id}}'>Fiche publique</a></div></div>""",a=a,counts=counts)
...
def android_operations():
 ...
 if pids:
  ...
  interventions=c.execute(f"""SELECT ot.id,ot.title,ot.task_type,ot.status,ot.priority,ot.start_at,ot.end_at,
   p.name project_name,z.name zone_name,t.name team_name,u.name assigned_name
   FROM operational_tasks ot LEFT JOIN projects p ON p.id=ot.project_id LEFT JOIN zones z ON z.id=ot.zone_id
   LEFT JOIN teams t ON t.id=ot.team_id LEFT JOIN users u ON u.id=ot.assigned_user_id
   WHERE (ot.project_id IN ({marks}) OR ot.project_id IS NULL) ORDER BY ot.start_at DESC""",pids).fetchall()
 else:
  zones=[];events=[];teams=[];missions=[];interventions=[]
 c.close()
 return jsonify({
  ...
  'interventions':[dict(x) for x in interventions]
 })

@app.get('/api/v1/associations/<int:aid>/module/<string:module>')
@android_auth
def android_association_module(aid,module):
 c=db(); uid=request.android_uid
 u=c.execute("SELECT role FROM users WHERE id=?",(uid,)).fetchone()
 super_admin=bool(u and u['role']=='super_admin')
 ...
 if not super_admin and not mem:
  c.close(); return jsonify({'error':{'message':'Permission insuffisante'}}),403
 items=[]
 if module=='members':
  rows=c.execute("SELECT u.name,m.role_code,m.status FROM association_memberships m JOIN users u ON u.id=m.user_id WHERE m.association_id=? AND m.status='approved' ORDER BY u.name",(aid,)).fetchall()
  items=[{'title':x['name'],'subtitle':x['role_code']+' · '+x['status']} for x in rows]
 elif module=='member_requests':
  rows=c.execute("SELECT u.name,m.member_kind,m.requested_at FROM association_memberships m JOIN users u ON u.id=m.user_id WHERE m.association_id=? AND m.status='pending' ORDER BY m.requested_at DESC",(aid,)).fetchall()
  items=[{'title':x['name'],'subtitle':x['member_kind']+' · '+x['requested_at']} for x in rows]
 elif module=='collaborations':
  rows=c.execute("""SELECT p.name project_name,a1.name inviting_name,a2.name invited_name,ac.status
   FROM association_collaborations ac JOIN projects p ON p.id=ac.project_id
   JOIN associations a1 ON a1.id=ac.inviting_association_id JOIN associations a2 ON a2.id=ac.invited_association_id
   WHERE ac.inviting_association_id=? OR ac.invited_association_id=? ORDER BY ac.id DESC""",(aid,aid)).fetchall()
  items=[{'title':x['project_name'],'subtitle':x['inviting_name']+' ↔ '+x['invited_name']+' · '+x['status']} for x in rows]
 elif module=='stock':
  rows=c.execute("""SELECT COALESCE(s.name_fr,'Espèce') species,ns.quantity_available,ns.quantity_reserved,ns.location
   FROM nursery_stock ns LEFT JOIN species s ON s.id=ns.species_id ORDER BY species""").fetchall()
  items=[{'title':x['species'],'subtitle':'Disponible: '+str(x['quantity_available'])+' · Réservé: '+str(x['quantity_reserved'])+((' · '+x['location']) if x['location'] else '')} for x in rows]
 elif module=='finance':
  rows=c.execute("""SELECT cm.movement_type,cm.amount,cm.category,cm.description,cm.created_at
   FROM cash_movements cm LEFT JOIN projects p ON p.id=cm.project_id
   WHERE p.association_id=? OR cm.project_id IS NULL ORDER BY cm.id DESC LIMIT 200""",(aid,)).fetchall()
  items=[{'title':x['movement_type']+' · '+str(x['amount'])+' DZD','subtitle':(x['category'] or '')+' · '+(x['description'] or '')+' · '+x['created_at']} for x in rows]
 else:
  c.close(); return jsonify({'error':{'message':'Module inconnu'}}),404
 c.close(); return jsonify({'module':module,'items':items})
